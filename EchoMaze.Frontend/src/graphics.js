...
// Stamina System
export let maxStamina = 100;
export let isExhausted = false;
let currentStamina = 100.0;
let isSprinting = false;
const staminaRegenRate = 15.0; // Per second
const staminaDrainRate = 35.0; // Per second

let lastSprintNoiseTime = 0;

export function popSprintNoise() {
    if (currentRole === "Survivor" && isSprinting && (performance.now() - lastSprintNoiseTime) > 500) {
        lastSprintNoiseTime = performance.now();
        return true;
    }
    return false;
}
...
const sharedUniforms = {
    uTime: { value: 0.0 },
    uSoundPositions: { value: Array.from({ length: maxSounds }, () => new THREE.Vector3()) },
...
// Survivor Specific Shader
const survivorFragShader = `
    uniform float uTime;
    uniform vec3 uSoundPositions[10];
    uniform float uSoundStartTimes[10];
    uniform float uSoundIntensities[10];
    ...
    void main() {
        ...
        for(int i = 0; i < 10; i++) {
            ...
            float timeAlive = uTime - uSoundStartTimes[i];
            if (timeAlive > 0.0 && timeAlive < maxLifetime) {
                float distToCamera = distance(uSoundPositions[i], uCameraPos);
                float timeSinceHit = timeAlive - (distToCamera / soundSpeed);
                
                // Has the wave reached the Monster?
                if (timeSinceHit > 0.0 && timeSinceHit < 1.5) {
                    float fade = 1.0 - (timeSinceHit / 1.5);
                    float attenuation = 1.0 / (1.0 + distToCamera * 0.1);
                    
                    float strength = fade * uSoundIntensities[i] * attenuation;
                    finalColor += vec3(1.0, 0.0, 0.0) * strength;
                    alpha += strength;
                }
            }
        }
...
// Survivor Material (Invisible until hit by wave, visible through walls)
const survivorEchoMaterial = new THREE.ShaderMaterial({
    ...
    `,
    fragmentShader: survivorFragShader
});
...
const MONSTER_SCALE = 0.6; 
const MONSTER_ROTATION_OFFSET = Math.PI;
...
function createMonsterModel() {
    if (monsterPrefab) {
        const group = new THREE.Group();
        const clonedScene = SkeletonUtils.clone(monsterPrefab);
        
        // Center the model or apply base offset
        clonedScene.position.y = 0;
        clonedScene.rotation.y = MONSTER_ROTATION_OFFSET;
        group.add(clonedScene);

        if (monsterAnimations && monsterAnimations.length > 0) {
            const mixer = new THREE.AnimationMixer(clonedScene);
            const action = mixer.clipAction(monsterAnimations[0]);
            // If we have an animation, we can control weight or play/pause
            action.play();
            action.setEffectiveWeight(0); // Pause basically until walking
        ...
        return createFallbackMonsterModel();
    }
}

function createFallbackMonsterModel() {
    const group = new THREE.Group();
...
export function initGraphics() {
    ...
    // Darker fog
    scene.fog = new THREE.Fog(0x000000, 2, 25);

    // GLTF Loading
    const gltfLoader = new GLTFLoader();
    gltfLoader.load('/models/monster.glb', (gltf) => {
        monsterPrefab = gltf.scene;
        monsterAnimations = gltf.animations;
        
        monsterPrefab.scale.set(MONSTER_SCALE, MONSTER_SCALE, MONSTER_SCALE);
        
        monsterPrefab.traverse(c => {
            if (c.isMesh) {
                c.userData.originalMaterial = c.material;
                c.castShadow = true;
            }
        });
        console.log("Monster GLTF loaded!");
        
        // If players already spawned with fallback, replace them
        for (const [id, m] of Object.entries(remotePlayerMeshes)) {
            if (serverState && serverState.players[id]) {
                const role = serverState.players[id].role || serverState.players[id].Role;
                if (role === "Monster") {
                    scene.remove(m);
                    const newM = createMonsterModel();
                    newM.userData = { ...m.userData, ...newM.userData };
                    scene.add(newM);
                    remotePlayerMeshes[id] = newM;
        ...
        console.error("Failed to load monster.glb", e);
    });
...
export function animate() {
    ...
    if (currentRole === "Monster") {
        ...
        const directionalContainer = document.getElementById("directional-indicators");
        if (directionalContainer) {
            directionalContainer.classList.remove("hidden");
            directionalContainer.innerHTML = ""; // Clear for this frame
            
            for (let i = 0; i < sharedUniforms.uSoundCount.value; i++) {
                const sPos = sharedUniforms.uSoundPositions.value[i];
                const sStart = sharedUniforms.uSoundStartTimes.value[i];
                const sInt = sharedUniforms.uSoundIntensities.value[i];
                
                const timeAlive = (Date.now() / 1000.0) - sStart;
                const maxLifetime = 5.0;
                const soundSpeed = 10.0;
                
                if (timeAlive > 0 && timeAlive < maxLifetime) {
                    const distToCamera = camera.position.distanceTo(sPos);
                    const timeSinceHit = timeAlive - (distToCamera / soundSpeed);
                    
                    if (timeSinceHit > 0 && timeSinceHit < 1.5) {
                        // Sound has hit the camera, show directional indicator
                        const dirToSound = new THREE.Vector3().subVectors(sPos, camera.position).normalize();
                        ...
                        dirToSound.y = 0; dirToSound.normalize();
                        
                        const dotForward = forward.dot(dirToSound);
                        const dotRight = right.dot(dirToSound);
                        
                        // We show it if it's outside FOV or if we want to show it always. Let's show it always, it helps localization.
                        const fade = 1.0 - (timeSinceHit / 1.5);
                        const opacity = fade * Math.min(sInt / 2.0, 1.0);
                        
                        // Angle: 0 is forward, positive is right. 
                        // In CSS, 0deg points UP. So an angle of 0 means translate straight up.
                        let angle = Math.atan2(dotRight, dotForward);
                        const angleDeg = angle * (180 / Math.PI);
                        
                        const indicator = document.createElement("div");
                        indicator.className = "sound-indicator";
                        // Rotate to angle, then translate out by 200px (so it circles the center), then rotate back to point outward?
                        // If transform origin is 0 0 (center), rotate(X) translateY(-200px) moves it to the edge pointing out!
                        indicator.style.transform = `rotate(${angleDeg}deg) translateY(-250px)`;
                        indicator.style.opacity = opacity.toFixed(2);
                        
                        // Scale size by intensity
                        const scale = 1.0 + Math.min(sInt / 5.0, 1.0);
                        indicator.style.transform += ` scale(${scale})`;
                        
                        directionalContainer.appendChild(indicator);
                    }
                }
            }
        }
    ...
    for (const id in remotePlayerMeshes) {
        ...
        if (m.userData.targetPosition) {
            // Models are placed at Y=0, targetPosition is camera height (Y=1)
            m.position.lerp(new THREE.Vector3(m.userData.targetPosition.x, m.userData.targetPosition.y - 1.0, m.userData.targetPosition.z), 0.2);
        }
        if (m.userData.targetRotation !== undefined) {
            // Smooth rotation interpolation using Quaternions to prevent wrapping bugs
            const targetQuat = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), m.userData.targetRotation);
            m.quaternion.slerp(targetQuat, 0.2);
        }
        
        // Procedural Animation OR GLTF Animation
        if (m.userData.isGLTF) {
            if (m.userData.mixer) {
                m.userData.mixer.update(delta);
                if (m.userData.walkAction) {
                    const targetWeight = m.userData.isWalking ? 1.0 : 0.0;
                    const currentWeight = m.userData.walkAction.getEffectiveWeight();
                    m.userData.walkAction.setEffectiveWeight(currentWeight + (targetWeight - currentWeight) * delta * 10);
                }
            }
        } else {
